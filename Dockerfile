...
CMD ["bash", "-lc", "python manage.py migrate --noinput && python manage.py collectstatic --noinput && WORKERS=\"${WEB_CONCURRENCY:-}\"; if [ -z \"$WORKERS\" ]; then WORKERS=\"$(python3 /app/tools/auto_concurrency.py web 2>/dev/null || true)\"; fi; case \"$WORKERS\" in (''|*[!0-9]*) WORKERS=2 ;; esac; exec gunicorn noctis_pro.asgi:application -k uvicorn.workers.UvicornWorker --bind 0.0.0.0:8000 --workers \"$WORKERS\" --timeout ${GUNICORN_TIMEOUT:-21600} --graceful-timeout ${GUNICORN_GRACEFUL_TIMEOUT:-30} --keep-alive ${GUNICORN_KEEPALIVE:-5}"]
