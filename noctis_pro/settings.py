...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                'admin_panel.context_processors.theme',
            ],
        },
    },
]
...
# Cache configuration
# Use Redis if available, fall back to local memory cache for dev
if REDIS_URL:
    CACHES = {
        'default': {
...
# Security settings
# SECURE_BROWSER_XSS_FILTER was removed from Django's SecurityMiddleware in Django 4.0
# (the X-XSS-Protection header is no longer sent). Removed to avoid misleading
# developers into thinking XSS filter enforcement is active.
